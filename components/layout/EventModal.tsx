...
export default function EventModal({
  ...
  const [endTime, setEndTime] = useState(
    editEvent?.end_time ?? defaultEndTime(defaultStart),
  );
  const [color, setColor] = useState(editEvent?.color ?? "blue");
  const [isRecurring, setIsRecurring] = useState(
    editEvent?.is_recurring ?? false,
  );
  ...
  const voiceFeedbackTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const applyParsed = (data: ParsedVoiceInput) => {
    ...
    if (data.tutorial !== null) {
      ...
      if (data.tutorial.isRecurring !== null)
        setTutorialIsRecurring(data.tutorial.isRecurring);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              {tutorialEnabled && (
                <div className="flex flex-col gap-3">
                  ...
                  <TimeRangeFields
                    startId="tut-start"
                    endId="tut-end"
                    startLabel="התחלת תרגול"
                    endLabel="סיום תרגול"
                    startValue={tutorialStart}
                    endValue={tutorialEnd}
                    onStartChange={(v) => {
                      setTutorialStart(v);
                      setTutorialEnd(defaultEndTime(v));
                    }}
                  ...
                  <RecurringCheckbox
                    ...
                    onChange={setTutorialIsRecurring}
                  />
                </div>
              )}
            </div>
          ...
          <div className="flex gap-2 justify-between pt-1">
            ...
            <div className="flex flex-col items-end gap-2 flex-1">
              {voiceFeedback && (
                <p className="w-full rounded-md px-3 py-1.5 text-xs bg-amber-50 text-amber-700 border border-amber-200 text-right">
                  {voiceFeedback}
                </p>
              )}
              <div className="flex gap-2">
