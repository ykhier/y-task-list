...
3. `SupabaseProvider` route guard: admin users without `otp_verified` in sessionStorage are signed out and sent to `/login`; this guard is skipped on all `AUTH_PATHS` to allow the login → verify-otp flow to complete. Admin status is cached in `sessionStorage` under the key `weekflow_admin_status:<userId>` to avoid repeated DB lookups.
...
`middleware.ts` runs on every request (excluding static assets) and calls `supabase.auth.getUser()` to refresh the session cookie — required for SSR auth to work correctly with `@supabase/ssr`. Routes under `/api/cron/` are excluded from the auth redirect so the cron job can be called without a session. The `?switch_account=1` query param clears OTP session storage (`otp_token`, `otp_verified`) and returns the user to `/login` — used when an admin wants to sign out during the OTP step.
...
- `lib/supabase/client.ts` — browser Supabase client; `lib/supabase/server.ts` — SSR client; `lib/supabase/admin.ts` — `createAdminClient()` (service role, server-only)
- `lib/materials/summary-document.ts` — `renderSummaryBodyHtml` (Markdown→HTML for in-panel display) + `buildSummaryDocumentHtml` (full RTL print doc for PDF). Add new Markdown constructs **only here**, never in components.
...
- `components/ui/PasswordInput.tsx` — show/hide toggle password field; use in all auth forms
- `components/layout/AnalyticsSummary.tsx` — stats card (completed today, weekly rate, events this week, calendar-synced task count); rendered inside TaskList
- `types/database.types.ts` — regenerate with `npx supabase gen types typescript --project-id <id>` after schema changes
- `lib/date.ts` — `generateDateOptions(anchor?)` (today + 13 days, used by task/event forms), `nextOccurrenceOfDay(dayIndex)` (next weekday occurrence, used to convert voice-parsed `dayIndex` to a real date), `timeToOffset` / `timeRangeToHeight` (calendar grid math)
...
`/admin` is a server component that requires `is_admin = true` (redirects otherwise). It uses `createAdminClient()` (service role) to list all auth users and join against `profiles`. `AdminUsersClient.tsx` renders the user table and calls API routes to toggle per-user flags.

Admin API routes (all require a valid session + admin check via service role):
- `GET /api/admin/users` — lists all auth users joined with `profiles`
- `PATCH /api/admin/users` — toggles `is_admin` or `digest_enabled` for a user

Settings API routes (all require a valid session):
- `GET/PATCH /api/settings/notification` — reads/writes `profiles.digest_enabled` + `profiles.notification_hour` for the current user
- `GET /api/settings/preview-digest` — renders tomorrow's digest HTML for the current user (used by the settings preview UI)
- `POST /api/settings/test-digest` — sends a real digest email to the current user immediately (for testing)
...
- **New async fetches must guard against stale responses** using the `requestIdRef` counter pattern already in `useTasks`/`useEvents`/`useTutorials` — copy it exactly. The pattern: increment `requestIdRef.current` before the fetch, capture the value in a local `const requestId`, and discard the response if `requestId !== requestIdRef.current` when it arrives.
...
- `pb-safe-area-or-6` — `padding-bottom: max(1.5rem, env(safe-area-inset-bottom, 0px))` (bottom sheet home indicator clearance)
- `no-scrollbar` — hides scrollbar while preserving scroll (mobile day-picker strip)
- `fade-in`, `slide-up`, `strikethrough` keyframe animations

### Upload size limit

`next.config.ts` sets `serverActions.bodySizeLimit: '50mb'` to support large file uploads in the materials tab. Don't lower this without checking the largest supported file type (PPTX can be large).
